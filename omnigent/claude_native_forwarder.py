...
def _log_dropped_delta(
    *,
    ...
    :param detail: HTTP status, error string, or exception to report.
    :returns: None.
    """
    _logger.debug(
        "Dropping Claude streamed delta after HTTP failure; session=%s "
        "bridge_dir=%s message_id=%s index=%s detail=%s",
        session_id,
        bridge_dir,
        delta.message_id,
        delta.index,
...
async def _forward_delta_run(
    ...
    *,
    ...
    try:
        ...
        for delta in deltas:
            _log_dropped_delta(
                session_id=session_id, bridge_dir=bridge_dir, delta=delta, detail=repr(exc)
            )
        return
    ...
    for delta in deltas:
        try:
            ...
            _log_dropped_delta(
                ...
                bridge_dir=bridge_dir,
                delta=delta,
                detail=_http_status_for_log(exc),
            )
